...
namespace Detail
{
	...
	void CGame::OnKeyDown( SDL_Keysym key )
	{
		...
		{
			app->PushState(GameMenuSingleton::Instance());
		}
	}
	...
	void CGame::OnResume()
	{
		LOG_INFO("GAME --- RESUME");

		IGameState::OnResume();
	}

	void CGame::AttachOnEvents()
	{
		...
		boost::signals2::connection mouse = Events::Mouse::Instance()->AttachOnMouseMove([=](int x, int y, int relX, int relY, bool Left,bool Right,bool Middle)
		{
